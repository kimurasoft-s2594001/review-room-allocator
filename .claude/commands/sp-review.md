...
割り当て案を出力する前に、以下のチェックリストで核実を行う：

```
【核実チェックリスト】
□ 説明者衝突: 同一チームが複数の部屋で同時にReviewされていないか？
□ PO衝突:    同一POが複数の部屋で同時に必要になっていないか？
□ Team合併:  1〜3票のTeamは他のTeamと合併できないか？（最重要）
□ 部屋数:    必要最小限の部屋数になっているか？（最重要）
□ 時間超過:  7票/180分を大幅に超える部屋がないか？（軟制約、警告のみ）
```

**核実で問題が見つかった場合:**
- **部屋数が多すぎる場合**: 小票数Teamの合併を検討する
- **説明者/PO衝突がある場合**: 部屋内の順序を調整する
- どうしても解消できない場合は、警告セクションに明記する
...
- Review総時間: **120分**（2時間）
- **1部屋あたりの最大チケット数: 4〜6票**（120分 ÷ 25分 ≈ 4.8票）
- POの部屋移動時間を考慮する必要あり
- ⚠️ 1部屋のチケット数が6票を超える場合は、部屋を分割することを検討
...
**【制約条件】**:
- 1部屋あたりの推奨チケット数: 4〜6票（平均20分/票 × 6票 = 120分 ≈ 2時間）
- 1部屋が6票を超える場合、または120分を超える場合は【時間超過警告】で警告を出力

**【部屋利用効率】**（重要）:
- **最小部屋数を最優先**: 部屋数を減らすことを最優先目標とする
- **異なるTeamの合併**: 票数が少ないTeam同士は**積極的に同一部屋に合併**する
  - 同一部屋内で異なるTeamを順番に配置すれば、説明者は順番に対応できる
  - 例：T2(2票) + T4(1票) + T5(3票) = 6票 → 1部屋に合併可能
- **孤票回避**: 1〜3票のTeamは他のTeamと合併を検討する
- **時間超過は軟制約**: 6〜7票程度の超過は許容（警告のみ出力）

```
❌ 悪い例: T1(4票)→部屋1、T2(2票)→部屋2、T4(1票)→部屋3、T5(3票)→部屋4
          → 4部屋使用、孤票多い

✅ 良い例: T1(4票)→部屋1、T2+T4+T5(6票)→部屋2、T3(3票)→部屋3
          → 3部屋で済む、異なるTeamは順番に対応
```

**【配置の基本手順】**:
1. まず各チームのチケット数を確認
2. **票数が少ないTeam（1〜3票）を特定し、合併候補としてグループ化**
3. 合併後の部屋構成を決定（目標：最小部屋数）
4. 同一部屋内で異なるTeamがある場合、Team単位で順番に配置（説明者が順番に対応できるよう）
5. 各Team内では主1 → 支援とPOのみ の順に並べる
6. POの移動経路を確認し、時間衝突がないかチェック
...
**出力順序の優先順位**:
1. **第一優先**: チーム順（T1 → T2 → T3 → T4 → T5）
2. **第二優先**: 各チーム内で種別順
   - 主1（`要（回覧）`）→ 優先度：高
   - 主0（`否`）→ 優先度：低

**その他のルール**:
- **フォーマット**: チーム見出しなし、直接チケットリストを出力
- **PO表示**:
...
{1部屋が6票を超える、または予想時間が120分を超える場合に警告を出力}
{該当なしの場合は「該当なし」と出力}
